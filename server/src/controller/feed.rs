...
use crate::{
    ...
    model::{
        db::FeedTypeMark,
        dto::{
            FeedAddReq, FeedFetchReq, FeedMoveReq, FeedSortReq, FeedsCountReq, FolderAddReq,
            FolderRenameReq, MarkAllUnreadReq,
        },
    },
};
...
pub async fn handle_get_subscribes(app_state: State<WebAppState>) -> impl IntoResponse {
    let result: Vec<serde_json::Value> = app_state
        ...
        .map(|e| {
            ...
            "create_date": e.create_time,
            "update_time": e.update_time,
            "parent_uuid": e.parent_id,
...
pub async fn handle_get_feed_log(
    ...
    let results = app_state.pool.get_feed_log(&feed_id).await;
...
pub async fn handle_mark_as_read(
    app_state: State<WebAppState>,
    Json(body): Json<MarkAllUnreadReq>,
) -> impl IntoResponse {
