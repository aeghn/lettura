...
export interface ArticleSlice {
  ...
  updateArticleStatus: (article: ArticleResItem, status: boolean) => any;
...
export const createArticleSlice: StateCreator<ArticleSlice & FeedSlice, [], [], ArticleSlice> = (
  set,
  get,
  ...args
) => ({
  ...
  updateArticleStatus: (article: ArticleResItem, status: boolean) => {
    if (article.is_read === status) {
      return Promise.resolve();
    }

    return dataAgent.updateArticleReadStatus(article.id, status).then((res) => {
      if (res) {
        let isToday = dayjs(dayjs(article.create_time).format("YYYY-MM-DD")).isSame(dayjs().format("YYYY-MM-DD"));

        if (status === true) {
          console.log("%c Line:80 🥪 true", "color:#f5ce50", true);
          get().updateCollectionMeta(isToday ? -1 : 0, -1);
          get().updateUnreadCount(article.feed_id, "decrease", 1);
        }

        if (status === false) {
          get().updateCollectionMeta(isToday ? 1 : 0, 1);
          get().updateUnreadCount(article.feed_id, "increase", 1);
        }
      }
    });
  },
