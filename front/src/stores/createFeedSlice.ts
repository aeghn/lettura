...
export interface FeedSlice {
  ...
  filterValue: number; // 0 star, 1 unread, 2 all
...
export const createFeedSlice: StateCreator<FeedSlice> = (set, get, ...args) => ({
  ...
  setFilterValue: (value: number | null) => {
    ...
    dataAgent.getUnreadTotal(v).then((m) => {
      get().feedList.map((feed) => {
        get().updateUnreadCount(feed.uuid, "set", m.data[feed.uuid]);
      });
    });
  },
  ...
  getFeedList: () => {
    ...
    return Promise.all([dataAgent.getFeeds(), dataAgent.getUnreadTotal(filter)]).then(
  ...
  setFeedContextMenuTarget: (target: FeedResItem | null) => {
    set(() => ({
  ...
  syncArticles(feed: FeedResItem): Promise<any> {
    ...
    const fns = (children && children?.length > 0 ? children : [{ ...feed }]).map((_) => {
      return limit(() => {
