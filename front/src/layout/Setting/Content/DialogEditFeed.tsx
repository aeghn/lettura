...
export const DialogEditFeed = React.memo((props: DialogEditFeedProps) => {
  ...
  const onUpdateInterval = (feed: FeedResItem | null, interval: string) => {
    if (feed) {
      if (/^\d*$/.test(interval)) {
        const numberValue = parseInt(interval, 10);
        dataAgent.updateFeedSyncInterval(feed.uuid, numberValue);
      }
    }
  };
      ...
      <DialogContent>
        <div className="py-6">
            ...
            <div>
              ...
              <div className="mt-3 space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">
                  Date subscribed: {Dayjs(feed?.create_time).format("YYYY-MM-DD HH:mm")}
                </p>
              </div>
            </div>
          </header>
          ...
          <div className="space-y-1 flex flex-col">
            <p className="flex-1 text-sm font-medium leading-none">Sync Interval</p>

            <Input
              type="number"
              min="360"
              max="999999"
              className="flex-1 text-sm font-normal  w-3/12"
              placeholder={feed?.sync_interval_sec.toString()}
              onChange={(e: { target: { value: string } }) => onUpdateInterval(feed, e.target.value)}
            />
          </div>
        </div>
        {/*<DialogFooter>*/}
