...
export function ResultItem(props: ResultItemProps) {
  ...
  function handleClick() {
    ...
    if (article.is_read === false) {
      dataAgent.updateArticleReadStatus(article.id, true).then(() => {
        article.is_read = true;
        console.log("%c Line:26 🍑 article.read_status", "color:#b03734", article.is_read);
        setReadStatus(true);
      });
    }
  }
  ...
  useEffect(() => {
    let match_img = (article.description || article.content)?.match(/<img.*?src="(.*?)"/);
  ...
  return (
    <div
      className={clsx(
        ...
        {
          "text-foreground": readStatus === false,
    ...
    >
      ...
      <div className="text-sm text-muted-foreground leading-6">{article.feed_title}</div>
