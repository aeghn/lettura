...
export const List = () => {
  ...
  const moveItem = (dragItem: TreeItem, dropResult: TreeItem, position: string | null): TreeItem[] => {
    ...
    console.log("%c Line:58 🍑 from folder to folder", "color:#ffdd4d", dragUuid, dropUuid);
    ...
    if (
      ...
      if (dragItemParent && dragItemParent.children) {
        dragItemParent.children = removeItem(dragItemParent.children, dragUuid);
        const children = (dropItemParent || dropResult).children;
        if (children) {
          const indexInFolder = children.findIndex((item) => item.uuid === dragUuid);
